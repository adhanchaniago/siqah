...
<table class="borded striped">
	...
	<?php
	$i = 1;
	$select = $koneksi->prepare("SELECT id_rekap, nama_kelompok, angkatan_kelompok from rekap WHERE nama_murabbi = '$nama_murabbi' GROUP BY nama_kelompok ORDER BY nama_kelompok ASC;");
				$select->execute();
				$select->store_result();
				$select->bind_result($id_rekap, $nama_kelompok, $angkatan_kelompok);
				while($select->fetch())
				{
				?>
	<tr>
    ...
    <td><?=$angkatan_kelompok;?></td>
		<td><a href="data_mutarabbi.php?nama=<?=$nama_kelompok;?>">Buka</a> | <a href="edit_kelompok.php?id=<?=$id_rekap;?>">Edit</a> | <a href="hapus_kelompok.php?nama=<?=$nama_kelompok;?>" onclick="return confirm('Yakin Ingin Hapus Kelompok & Isinya?');">Hapus</a></td>
	</tr>
	<?php
				}
				?>
