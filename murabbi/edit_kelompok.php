...
//Dapatkan user
$select = $koneksi->prepare("SELECT nama_kelompok FROM rekap WHERE id_rekap = $id_rekap;");
$select->execute();
$select->store_result();
...
//Dapatkan angkatan kelompok
$select = $koneksi->prepare("SELECT angkatan_kelompok, jenis_kelamin, jenjang FROM rekap WHERE nama_kelompok='$nama_kelompok';");
$select->execute();
$select->store_result();
$select->bind_result($angkatan_kelompok_lama, $jenis_kelamin, $jenjang_lama);
$select->fetch();
    ...
    <nav class="hide-on-med-and-down">
      ...
      <div class="container">
          <div class="col s12">
            ...
            <a href="" class="breadcrumb">Edit Kelompok</a>
          </div>
        </div>
      </div>
    </nav>     
...
<div class="container">
<h3>Edit Kelompok</h3>
<form method="post" action="proses_edit_kelompok.php" onsubmit="return confirm('Yakin Ingin Mengedit Kelompok?');">
	<div class="row">
    ...
    <div class="row">
        <div class="input-field col s12">
            <input type="hidden" name="nama_kelompok_lama" value="<?= stripslashes($nama_kelompok);?>" placeholder="Nama Kelompok Lama">
        	<input type="text" name="nama_kelompok_baru" value="<?= stripslashes($nama_kelompok);?>" placeholder="Nama Kelompok Baru">
			<label>Nama Kelompok</label>
        </div>
    </div>
    <div class="row">
        <div class="input-field col s12">
        <select name="jenjang_baru" id="jenjang_lama">
        <?php
        $select = $koneksi->prepare("SELECT id_jenjang FROM jenjang ORDER BY id_jenjang ASC;");
                    $select->execute();
                    $select->store_result();
                    $select->bind_result($jenjang_db);
                    while($select->fetch())
                    {
                    ?>
            <option value="<?=$jenjang_db;?>"><?=$jenjang_db;?></option>
            <?php
            }?>
        </select>
        <label>Kelas</label>
    ...
    <div class="row">
        <div class="input-field col s12">
            <select name="angkatan_kelompok_baru" id="angkatan_kelompok_lama">
    ...
    <button type="submit" class="waves-effect waves-light btn z-depth-3" name="edit" value="Edit">Edit</button>
</form>
</div>
</br>
...
<footer class="page-footer teal darken-4">
    ...
    <script>
        ...
        $('#angkatan_kelompok_lama option[value="<?=$angkatan_kelompok_lama;?>"]').attr('selected','selected');
        $('#jenis_kelamin option[value="<?=$jenis_kelamin;?>"]').attr('selected','selected');
        $('#jenjang_lama option[value="<?=$jenjang_lama;?>"]').attr('selected','selected');
    </script>
</body>
</html>