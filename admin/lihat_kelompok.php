...
//Dapatkan user
$select = $koneksi->prepare("SELECT nama_murabbi, jenis_kelamin FROM murabbi WHERE id_murabbi='$id_murabbi';");
$select->execute();
$select->store_result();
...
Jumlah: <?=$jumlah_kelompok;?> kelompok
<table class="borded striped">
	<tr>
		<th>No.</th>
		<th>Nama Kelompok</th>
    <th>Angkatan Kelompok</th>
		<th>Tindakan</th>
    <th>Menu</th>
	</tr>
	<?php
	$i = 1;
	$select = $koneksi->prepare("SELECT nama_kelompok, angkatan_kelompok from rekap WHERE nama_murabbi = '$nama_murabbi' GROUP BY nama_kelompok ORDER BY nama_kelompok ASC;");
				$select->execute();
				$select->store_result();
				$select->bind_result($nama_kelompok, $angkatan_kelompok);
				while($select->fetch())
				{
				?>
	<tr>
		<td><?=$i++;?></td>
		<td><?=$nama_kelompok;?></td>
    <td><?=$angkatan_kelompok;?></td>
		<td><a href="data_mutarabbi.php?nama=<?=$nama_kelompok;?>&angkatan_kelompok=<?=$angkatan_kelompok;?>">Buka</a> | <a href="edit_kelompok.php?nama=<?=$nama_kelompok;?>">Edit</a> | <a href="hapus_kelompok.php?nama=<?=$nama_kelompok;?>" onclick="return confirm('Yakin Ingin Hapus Kelompok & Isinya?');">Hapus</a></td>
    <td><a href="hasil_rekap_presensi.php?nama=<?=$nama_kelompok;?>">Rekap Presensi</a> | <a href="qadhaya.php?nama=<?=$nama_kelompok;?>">Qadhaya</a> | <a href="rekap_madah.php?nama=<?=$nama_kelompok;?>">Rekap Madah</a></td>
	</tr>
	<?php
				}
				?>
  ...
  <a href="tambah_kelompok.php?nama=<?=$nama_murabbi;?>" class="btn-floating btn-large waves-effect waves-light red"><i class="material-icons">add</i></a><label> Tambah Kelompok</label>
</div><br>
