...
$select->fetch();
	...
	$edit = $koneksi->prepare("UPDATE `mutarabbi` SET `status`='Aktif' WHERE id_mutarabbi='$id_mutarabbi' and nama_murabbi='$nama_murabbi'");
	$edit->execute();
	
	$sudah_edit = true;
	if($sudah_edit == true){
		echo '<script language="javascript">alert("BERHASIL AKTIFKAN MUTARABBI"); document.location="data_mutarabbi.php?nama=' . $nama_kelompok . '&angkatan_kelompok=' . $angkatan_kelompok . '";</script>';
	}
}
?>