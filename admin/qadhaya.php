...
<table class="border striped">
	...
	<?php
	$i = 1;
	$select = $koneksi->prepare("select id_laporan, pekan, bulan, tahun, qadhaya from laporan where nama_kelompok='$nama_kelompok' GROUP BY pekan, bulan, tahun ORDER BY id_laporan ASC");
				$select->execute();
				$select->store_result();
				$select->bind_result($id_laporan, $pekan, $bulan, $tahun, $qadhaya);
				while($select->fetch())
				{
				?>
	<tr>
    ...
    <td><?=$tahun;?></td>
		<td><?=$qadhaya;?></td>
		<td><a href="personal.php?id=<?=$id_laporan;?>">Personal</a></td>
	</tr>
	<?php
				}
				?>
