...
$select = $koneksi->prepare("SELECT nama_murabbi, nama_kelompok, pekan, bulan, tahun, madah from laporan WHERE id_laporan='$id_laporan'");
$select->execute();
$select->store_result();
$select->bind_result($nama_murabbi, $nama_kelompok, $pekan, $bulan, $tahun, $madah);
$select->fetch();
    ...
    <nav class="hide-on-med-and-down">
      ...
      <div class="container">
          <div class="col s12">
            ...
            <a href="personal_madah.php?id=<?=$id_laporan;?>" class="breadcrumb">Madah Personal</a>
          </div>
        </div>
      </div>
    </nav>
...
Bulan: <?=$bulan;?><br>
Tahun: <?=$tahun;?><br>
Madah Kelompok: <?=stripslashes($madah);?><br>
<h4>Mutarabbi yang Hadir:</h4>
...
<table class="border striped">
	...
	<?php
	$i = 1;
	$select = $koneksi->prepare("select nama_mutarabbi, keterangan from laporan where nama_kelompok='$nama_kelompok' and pekan='$pekan' and bulan='$bulan' and tahun='$tahun' and madah = '$madah' and kehadiran = 'Ya' ORDER BY nama_mutarabbi ASC");
				$select->execute();
				$select->store_result();
