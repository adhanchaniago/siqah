...
//Dapatkan nama murabbi, jenjang
$select = $koneksi->prepare("SELECT nama_murabbi, jenjang from rekap WHERE nama_kelompok='$nama_kelompok'");
$select->execute();
$select->store_result();
$select->bind_result($nama_murabbi, $jenjang);
$select->fetch();
    ...
    <nav class="hide-on-med-and-down">
      ...
      <div class="container">
          <div class="col s12">
            ...
            <a href="rekap_madah.php?nama=<?=$nama_kelompok;?>" class="breadcrumb">Madah Kelompok</a>
          </div>
        </div>
      </div>
    </nav>
...
<table class="border striped">
	...
	<?php
	$i = 1;
	$select = $koneksi->prepare("select laporan.id_laporan, mutarabbi.id_mutarabbi, laporan.pekan, laporan.bulan, laporan.tahun, laporan.madah from laporan, mutarabbi where laporan.nama_kelompok='$nama_kelompok' and mutarabbi.nama_mutarabbi = laporan.nama_mutarabbi GROUP BY laporan.pekan, laporan.bulan, laporan.tahun ORDER BY id_laporan ASC");
				$select->execute();
				$select->store_result();
				$select->bind_result($id_laporan, $id_mutarabbi, $pekan, $bulan, $tahun, $madah);
				while($select->fetch())
				{
				?>
	<tr>
    ...
    <td><?=$tahun;?></td>
		<td><?=$madah;?></td>
		<td><a href="personal_madah.php?id=<?=$id_laporan;?>">Personal</a></td>
	</tr>
	<?php
				}
				?>
